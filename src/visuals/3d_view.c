...
void draw_vertical_line(t_data *game_data, float dist, int ray)
{
	...
	height = (20 * MAP_HEIGHT) / dist;
	float y1 = (MAP_HEIGHT / 2) - height / 2;
	int x = (MAP_WIDTH / MAP_WIDTH) * ray;
	int i = 0;
	while (i < 10)
	{
		other_draw_line(game_data, x+i, y1, x+i, y1+height, SCARLET_COLOR);
		i++;
	}
}
...
/*void ft_put_3dview(t_data *game_data)
{
	...
	game_data->win = mlx_new_window(game_data->mlx, MAP_WIDTH, MAP_WIDTH, "Cube3D");
	...
	mlx_put_image_to_window(game_data->mlx, game_data->win, final, MAP_WIDTH/2 - 300/2, 100);
