...
/*   Updated: 2024/04/18 11:52:50 by tmalidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	left_right_move(int key, t_data *gd)
{
	if (key == ROTATE_LEFT)
	{
		gd->pos_y -= (gd->x2 * sin(gd->copy_angle + 90 * M_PI / 180)
				+ gd->y2 * cos(gd->copy_angle + 90 * M_PI / 180)) / (RAY_DISTANCE / 2) * 3;
		gd->pos_x -= (gd->x2 * cos(gd->copy_angle + 90 * M_PI / 180)
				- gd->y2 * sin(gd->copy_angle + 90 * M_PI / 180)) / (RAY_DISTANCE / 2) * 3;
	}
...
void	front_back_move(int key, t_data *gd)
{
	if (key == 119)
	{
		gd->pos_y += (gd->x2 * sin(gd->copy_angle)
				+ gd->y2 * cos(gd->copy_angle)) / (RAY_DISTANCE / 2) * 3;
		gd->pos_x += (gd->x2 * cos(gd->copy_angle)
				- gd->y2 * sin(gd->copy_angle)) / (RAY_DISTANCE / 2) * 3;
	}
	else if (key == 115)
	{
		gd->pos_y -= (gd->x2 * sin(gd->copy_angle)
