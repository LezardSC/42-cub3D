...
/*   Updated: 2024/03/20 17:58:36 by tmalidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *new_display(t_data *game_data, char *wall_data, int ray)
{
	...
	while (y < 1080)
	{
		...
		while (i < data.bpp / 8)
		{
			if (data.original_index + i < data.original_width * data.original_height
				* (data.bpp / 8) && data.enlarged_index + i < 1 * 1080 * (data.bpp / 8))
				data.enlarged_wall_data[data.enlarged_index + i] = wall_data[data.original_index + i];
			i++;
		}
		y++;
	}
